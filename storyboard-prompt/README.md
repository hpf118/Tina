...
| `Validation_Study_Design_Storyboard_Master_Prompt.md` / `.docx` | Program-evaluation design for validating the prompt: CIPP architecture, an interpretation-and-use argument with success criteria per claim, implementation outcomes as the stakeholder lens, mixed-methods procedures, and the instruments (designer questionnaire, interview guides, developer rubric, client reviewer questionnaire, metrics sheet, reproducibility protocol). |
| `AI_Storyboarding_Manuscript_Reconstructed.md` / `.docx` | The draft paper "Transforming Learning Design at Scale", transcribed from 12 screenshots (screens 1 to 36 of 37) into an editable Word document with the blue/green/purple/red editorial markers preserved; a closing Transcription Notes section lists what to verify. Rebuild with `node build-paper-docx.js AI_Storyboarding_Manuscript_Reconstructed.md out.docx`. |
| `Integrated_Study_Plan_Development_and_Validation.md` / `.docx` | Assessment of whether the draft paper and the validation design form one study (recommendation: two linked papers), the checklist for bringing the draft to a publishable standard, the full manuscript plan for the validation paper with highlighted data placeholders, the data-collection register, the single-paper alternative, and the schedule. |
| `build-paper-docx.js` | Markdown to Word builder for manuscripts: Times New Roman, colour rules for the editorial markers, inline `[DATA TO COLLECT: ...]`, `[RESULT: ...]`, `[INSERT: ...]` placeholders rendered red on yellow, `@ref` hanging-indent references, `@center`, `@caption`, `@pagebreak`. |
